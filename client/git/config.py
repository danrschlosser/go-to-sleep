...
class MetaParserBuilder(type):
    ...
    def __new__(metacls, name, bases, clsdict):
        ...
        if kmm in clsdict:
            ...
            for base in bases:
                methods = ( t for t in inspect.getmembers(base, inspect.ismethod) if
...
class SectionConstraint(object):
    ...
    _valid_attrs_ = (
    "get_value", "set_value", "get", "set", "getint", "getfloat", "getboolean", "has_option",
...
class GitConfigParser(cp.RawConfigParser, object):
    ...
    OPTCRE = re.compile(
        ...
        r'\s*(?P<vi>[:=])\s*'  # any number of space/tab,  # followed by separator  # (either : or =), followed  # by any # space/tab
    ...
    __slots__ = (
    "_sections", "_defaults", "_file_or_files", "_read_only", "_is_initialized", '_lock')
    ...
    def get_value(self, section, option, default=None):
        ...
        if not isinstance(valuestr, basestring):
            raise TypeError("Invalid value type: only int, long, float and str are allowed",
                            valuestr)
