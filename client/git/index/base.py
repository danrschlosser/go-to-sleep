...
class IndexFile(LazyMixin, diff.Diffable, Serializable):
    ...
    def _to_relative_path(self, path):
        ...
        if relative_path == path:
            raise ValueError("Absolute path %r is not in git repository at %r" % (
    ...
    def add(self, items, force=True, fprogress=lambda *args: None, path_rewriter=None,
        ...
        if entries:
            ...
            null_entries_indices = [i for i, e in enumerate(entries) if
                                    e.binsha == Object.NULL_BIN_SHA]
    ...
    def checkout(self, paths=None, force=False, fprogress=lambda *args: None, **kwargs):
        ...
        def handle_stderr(proc, iter_checked_out_files):
            ...
            unknown_lines = list()
            endings = (
            ' already exists', ' is not in the cache', ' does not exist at stage', ' is unmerged')
