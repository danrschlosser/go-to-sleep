...
class UpdateProgress(RemoteProgress):
    ...
    CLONE, FETCH, UPDWKTREE = [1 << x for x in range(RemoteProgress._num_op_codes,
                                                     RemoteProgress._num_op_codes + 3)]
...
class Submodule(util.IndexObject, Iterable, Traversable):
    ...
    def __init__(self, repo, binsha, mode=None, path=None, name=None, parent_commit=None, url=None,
                 branch_path=None):
    ...
    def _set_cache_(self, attr):
        if attr == '_parent_commit':
            ...
            self._branch_path = reader.get_value(self.k_head_option,
                                                 git.Head.to_full_path(self.k_head_default))
        elif attr == '_name':
    ...
    def add(cls, repo, name, path, url=None, branch=None, no_checkout=False):
        ...
        if url is None:
            if not has_module:
                raise ValueError(
                    "A URL was not given and existing repository did not exsit at %s" % path)
            # END check url
            mrepo = sm.module()
    ...
    def update(self, recursive=False, init=True, to_latest_revision=False, progress=None,
        ...
        try:
            ...
            if not dry_run and os.path.isdir(module_path):
                try:
                    ...
                    raise OSError(
                        "Module directory at %r does already exist and is non-empty" % module_path)
                # END handle OSError
            ...
            if not dry_run:
                ...
                try:
                    ...
                    mrepo.head.set_reference(local_branch,
        ...
        if mrepo is not None and mrepo.head.commit.binsha != binsha:
            progress.update(BEGIN | UPDWKTREE, 0, 1,
                            prefix + "Updating working tree at %s for submodule %r to revision %s" % (
            ...
            if not dry_run:
                ...
                # END handle checkout
            #END handle dry_run
            progress.update(END | UPDWKTREE, 0, 1,
