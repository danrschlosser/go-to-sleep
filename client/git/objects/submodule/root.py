...
class RootUpdateProgress(UpdateProgress):
    ...
    REMOVE, PATHCHANGE, BRANCHCHANGE, URLCHANGE = [1 << x for x in
                                                   range(UpdateProgress._num_op_codes,
                                                         UpdateProgress._num_op_codes + 4)]
    ...
    def update(self, previous_commit=None, recursive=True, force_remove=False, init=True,
        ...
        for i, csm in enumerate(csms):
            ...
            if sm.path != psm.path and psm.module_exists():
                progress.update(BEGIN | PATHCHANGE, i, len_csms,
                                prefix + "Moving repository of submodule %r from %s to %s" % (
                                sm.name, psm.abspath, sm.abspath))
                # move the module to the new path
                if not dry_run:
                    psm.move(sm.path, module=True, configuration=False)
                #END handle dry_run
                progress.update(END | PATHCHANGE, i, len_csms,
            ...
            if sm.module_exists():
                ...
                if sm.url != psm.url:
                    ...
                    if len([r for r in rmts if r.url == sm.url]) == 0:
                        progress.update(BEGIN | URLCHANGE, i, len_csms,
                                        prefix + "Changing url of submodule %r from %s to %s" % (
                        ...
                        #END handle dry_run
                        progress.update(END | URLCHANGE, i, len_csms,
                ...
                if sm.branch_path != psm.branch_path:
                    ...
                    # new remote branch
                    progress.update(BEGIN | BRANCHCHANGE, i, len_csms,
                                    prefix + "Changing branch of submodule %r from %s to %s" % (
                    ...
                    progress.update(END | BRANCHCHANGE, i, len_csms,
