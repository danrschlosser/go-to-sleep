        ...
        * means birth of new branch or tag
        ...
        if ref_type is SymbolicReference:
            ...
            if remote_local_ref.startswith(Reference._common_path_default + "/"):
                ...
                if ref_type is not TagReference and not remote_local_ref.startswith(
...
class Remote(LazyMixin, Iterable):
    ...
    def _set_cache_(self, attr):
        if attr == "_config_reader":
            self._config_reader = SectionConstraint(self.repo.config_reader(),
    ...
    def fetch(self, refspec=None, progress=None, **kwargs):
        ...
        proc = self.repo.git.fetch(self, refspec, with_extended_output=True, as_process=True,
                                   v=True, **kwargs)
    ...
    def pull(self, refspec=None, progress=None, **kwargs):
        ...
        kwargs = add_progress(kwargs, self.repo.git, progress)
        proc = self.repo.git.pull(self, refspec, with_extended_output=True, as_process=True, v=True,
                                  **kwargs)
        return self._get_fetch_info_from_stderr(proc, progress or RemoteProgress())
