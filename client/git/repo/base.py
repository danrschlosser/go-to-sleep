...
class Repo(object):
    ...
    def working_tree_dir(self):
        ...
        if self._working_tree_dir is None:
            raise AssertionError(
                "Repository at %r is bare and does not have a working tree directory" % self.git_dir)
    ...
    def untracked_files(self):
        ...
        for line in stream:
            ...
            for untracked_info in stream:
                matches_info = regex.match(untracked_info)
                untracked_info = matches_info.group(1)
                untracked_files.append(untracked_info.rstrip())
    ...
    def _clone(cls, git, url, path, odb_default_type, progress, **kwargs):
        ...
        try:
            proc = git.clone(url, path, with_extended_output=True, as_process=True, v=True,
        ...
        if repo.remotes:
            repo.remotes[0].config_writer.set_value('url', repo.remotes[0].url.replace("\\\\",
                                                                                       "\\").replace(
                "\\", "/"))
