...
class TestIndex(TestBase):
    ...
    def test_index_file_diffing(self, rw_repo):
        ...
        try:
            ...
            assert (len(e.failed_files) == len(e.failed_reasons)) and isinstance(
                e.failed_reasons[0], basestring)
    ...
    def test_index_mutation(self, rw_repo):
        ...
        commit_multi_parent = index.commit(commit_message,
                                           parent_commits=(commit_no_parents, new_commit))
        ...
        # glob
        entries = index.reset(new_commit).add([os.path.join('lib', 'git', '*.py')],
        ...
        entries = index.reset(new_commit).add(
            [os.path.abspath(os.path.join('lib', 'git', 'head.py'))] * 2,
            fprogress=self._fprogress_add)
        self._assert_entries(entries)
        ...
        null_bin_sha = "\0" * 20
        self.failUnlessRaises(ValueError, index.reset(new_commit).add,
        ...
        entries = index.reset(new_commit).add(
            [BaseIndexEntry((010644, null_bin_sha, 0, new_file_relapath))],
            fprogress=self._fprogress_add)
        self._assert_entries(entries)
        self._assert_fprogress(entries)
        ...
        full_index_entry = IndexEntry.from_base(
            BaseIndexEntry((0120000, entries[0].binsha, 0, entries[0].path)))
