...
if sys.platform == 'win32':
    try:
        ...
        sys.stderr.write(
            "The submodule tests will fail as some files cannot be removed due to open file handles.\n")
        sys.stderr.write(
...
class TestSubmodule(TestBase):
    ...
    def _do_base_tests(self, rwrepo):
        ...
        if rwrepo.bare:
            ...
            self.failUnlessRaises(ValueError, Submodule.add, rwrepo, "newsubm", "pathtorepo",
                                  url=None)
            ...
            # or we raise
            self.failUnlessRaises(ValueError, Submodule.add, rwrepo, "newsubm", sm.path,
            ...
            new_csmclone_path = to_native_path_linux(
                join_path_native(self.rorepo.working_tree_dir, sm.path, csm.path))
            ...
            smid = "newsub"
            osmid = "othersub"
            nsm = Submodule.add(rwrepo, smid, sm_repopath, new_smclone_path + "/", None,
                                no_checkout=True)
            ...
            assert not os.path.isfile(
                join_path_native(nsm.module().working_tree_dir, Submodule.k_modules_file))
            ...
            #############
            # invalid inptu
            self.failUnlessRaises(ValueError, nsm.move, 'doesntmatter', module=False,
                                  configuration=False)
    ...
    def test_root_module(self, rwrepo):
        ...
        sm.config_writer().set_value('url', to_native_path_linux(
            join_path_native(self.rorepo.working_tree_dir, sm.path)))
        ...
        sm.update(recursive=False)
        assert sm.module_exists()
        sm.config_writer().set_value('path',
        ...
        csmadded = rwrepo.index.commit(
            "Added submodule").hexsha  # make sure we don't keep the repo reference
